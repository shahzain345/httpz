"""Sequential GET benchmark: one client, N back-to-back GETs.

Measures the steady-state per-request overhead each library adds once the
connection pool is warm. The client is constructed once, a few warmup requests
prime it, then N requests are timed individually.
...
def time_sync_seq(name, client, get_call, url, count, warmup):
    progress_start(name)
...
async def time_async_seq(name, client, get_call, url, count, warmup):
    progress_start(name)
    for _ in range(warmup):
        try:
...
def main():
    args = parse_args("Sequential GET benchmark (1 client, N back-to-back GETs)")
    url = args.url or DEFAULT_GET_URL

    print_header("SEQUENTIAL GETs", args.count, url)
